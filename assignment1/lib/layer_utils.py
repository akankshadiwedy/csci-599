...
class flatten(object):
    ...
    def forward(self, feat):
        ...
        input_dim = np.prod(feat.shape[1:])
        output = feat.reshape([input_bz,input_dim])
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True):
        ...
        kept = np.random.binomial(1,self.keep_prob, size=feat.shape) 
        if is_training:
            if self.keep_prob != 0:
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        m = label.shape[0]
        predictions = -np.log(logit[range(m),label])
        loss = np.sum(predictions) / m
...
def softmax(feat):
    ...
    #scores = np.exp(feat) / np.sum(np.exp(feat))
    scores = np.exp((feat.T - np.max(feat, axis=1)).T)
    scores = (scores.T / scores.sum(axis=1)).T
