...
class LSTM(object):
    ...
    def step_forward(self, x, prev_h, prev_c):
        ...
        #############################################################################
        i, f, o, g = None, None, None, None
        output = prev_h.dot(self.params[self.wh_name]) + x.dot(self.params[self.wx_name]) + self.params[self.b_name]
        H = self.h_dim
        i = sigmoid(output[:,0:H])
        f = sigmoid(output[:,H:2*H])
        o = sigmoid(output[:,2*H: 3*H])
        g = np.tanh(output[:,3*H: 4*H])     
        next_c = f * prev_c + i * g
        tanh_c = np.tanh(next_c)
        next_h = o * tanh_c
        meta = [x, prev_h, prev_c, i, f, o, g, tanh_c]
        #############################################################################
        #                               END OF YOUR CODE                            #
        #############################################################################
    ...
    def step_backward(self, dnext_h, dnext_c, meta):
        ...
        #############################################################################
        x, prev_h, prev_c, i, f, o, g, tanh_c = meta 
        do = tanh_c * dnext_h
        dox = (1 - o) * o * do
        dc = dnext_h * o * (1-tanh_c**2) + dnext_c
        df = prev_c * dc
        dfx = (1 - f) * f * df
        di = g * dc
        dix = (1 - i) * i * di 
    ...
    def forward(self, x, h0):
        ...
        h = np.zeros((x.shape[0],x.shape[1],h0.shape[1]))
        c_zero = np.zeros_like(h0)
        next_c = None
        h[:,0,:], next_c, meta_i = self.step_forward(x[:,0,:], h0, c_zero)
        self.meta.append(meta_i)
        for i in range(1, x.shape[1]):
            c_zero = next_c
            h[:,i,:], next_c, meta_i = self.step_forward(x[:,i,:], h[:,i-1,:], c_zero)
            self.meta.append(meta_i)
        #############################################################################
        #                               END OF YOUR CODE                            #
        #############################################################################
    ...
    def backward(self, dh):
        ...
        dnext_h = dh[:,T-1,:]
        for i in reversed(range(0, T)):
            dx[:,i,:], dh_idx, dc_idx, dWx_idx, dWh_idx, db_idx = self.step_backward(dnext_h, dnext_c, self.meta[i])
            dnext_h = dh_idx + dh[:,i-1,:]
            dnext_c = dc_idx + dc[:,i-1,:]
            '''if i == 0:
                dh0 = dh_idx
            else:
                dnext_h = dh_idx + dh[:,i-1,:]
                dnext_c = dc_idx + dc[:,i-1,:]'''
            self.grads[self.wx_name] += dWx_idx
            self.grads[self.wh_name] += dWh_idx
            self.grads[self.b_name] += db_idx
        dh0 = dh_idx
        #############################################################################
        #                               END OF YOUR CODE                            #
        #############################################################################
